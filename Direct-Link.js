...
(() => {
  ...
  const hostRegex = /([\w.]{0,})(jianshu|juejin|sspai|youtube|leetcode|oschina|gitee|epicgames|adjust|shop-links|landiannews).(com|cn|org|net|co)/;
  ...
  function youtubeDirect() {
    function run() {
      ...
      timeoutID = setTimeout(() => {
        ...
        document.getElementById('description-inner').onmousemove = () => { run(); };
  ...
  (() => {
    ...
    const adjust = /([\w.]{0,})(hoyolab|mozilla|firefox).(com|org)/.test(CUR_HOST);
    const usingTarget = /([\w.]{0,})(juejin|leetcode|gitee|sspai).(com|cn)/.test(CUR_HOST);
    const isLandian = CUR_HOST.includes('landiannews.com');
    switch (true) {
      case CUR_HOST.includes('landiannews.com'):
      ...
      case adjust:
        ...
        break;
      case CUR_HOST.includes('jianshu.com'):
        indexParam = INDEX_TO;
      ...
      case usingTarget:
        ...
        linkDirect(indexParam, DELAY_TIME.normal);
        break;
      case CUR_HOST.includes('youtube.com'):
        indexParam = INDEX_YOUTUBE_Q;
        youtubeDirect();
        break;
      case CUR_HOST.includes('epicgames.com'):
        indexParam = INDEX_REDIRECTTO;
        linkDirect(indexParam, DELAY_TIME.normal);
        break;
      case CUR_HOST.includes('oschina.net'):
        indexParam = INDEX_GOTOPAGE;
        linkDirect(indexParam, DELAY_TIME.normal);
        break;
      case CUR_HOST.includes('xda-developers.com'):
