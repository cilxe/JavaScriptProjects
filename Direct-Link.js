...
- oschina.net
- gitee.com
- xda-developers.com (shop-links.co)
- sspai.com
- gcores.com
...
(() => {
  ...
  const INDEX_TARGET = ['target']; // Juejin, leet-code, gitee, sspai gcores
  ...
  const hostRegex = /([\w.]{0,})(jianshu|juejin|youtube|leetcode|oschina|gitee|sspai|gcores|epicgames|adjust|shop-links|landiannews).(com|cn|org|net|co)/;
  ...
  function landianDirect(delayTime) {
    clearTimeout(timeoutID);
    timeoutID = setTimeout(() => {
      const links = document.getElementsByTagName('a');
      for (let i = 0; i < links.length; i += 1) {
        if (hostRegex.test(links[i].hostname) && links[i].href.includes(INDEX_OURL[0])) {
  ...
  function youtubeDirect() {
    function run() {
      ...
      timeoutID = setTimeout(() => {
        ...
        document.getElementById('description-inner').onmousemove = () => { linkDirect(INDEX_YOUTUBE_Q, 0); };
      }, DELAY_TIME.normal + 600);
    }
    window.onload = () => {
      run();
      if (document.visibilityState === 'visible') { run(); }
      document.onvisibilitychange = () => { run(); };
    };
  }
  ...
  (() => {
    ...
    const adjust = /([\w.]{0,})(hoyolab|mozilla|firefox).(com|org)/.test(pageHost);
    const usingTarget = /([\w.]{0,})(juejin|leetcode|gitee|sspai|gcores).(com|cn)/.test(pageHost);
    const isLandian = pageHost.includes('landiannews.com');
    switch (true) {
      case pageHost.includes('landiannews.com'):
      ...
      case adjust:
        ...
        break;
      case pageHost.includes('jianshu.com'):
        indexParam = INDEX_TO;
      ...
      case pageHost.includes('oschina.net'):
        indexParam = INDEX_GOTOPAGE;
        linkDirect(indexParam, DELAY_TIME.normal);
        break;
      case pageHost.includes('xda-developers.com'):
        indexParam = INDEX_SHOPLINK;
        linkDirect(indexParam, DELAY_TIME.normal);
        break;
      default:
        break;
...
/*
v0.1.5.2 2023.05
- Add sspai|gcores.com (target).
- Add another redirecting index param for oschina.net.

v0.1.5 2023.05.05
- Errors fixes and code reduction.
- Replacing the shop-links to direct links on `xda-developers.com`.
