...
(() => {
      ...
      setTimeout(() => {
        ...
        switch (true) {
          case window.location.pathname === '/watch':
            redirectLinks = document.getElementsByClassName('yt-core-attributed-string__link');
            break;
          case window.location.pathname.includes('@'): // channels
            redirectLinks = document.getElementsByClassName('yt-simple-endpoint');
            break;
          default:
            break;
        }
        for (let i = 0; i < redirectLinks.length; i += 1) {
          if (urlRegex.test(redirectLinks[i].href) && redirectLinks[i].href.includes('youtube.com')) {
            const url = new URL(redirectLinks[i].href);
            const params = url.searchParams;
            if (params.has(urlParam)) { redirectLinks[i].href = decodeURIComponent(params.get(urlParam)); }
          }
        }
      }, delayTime);
    }
  ...
  function mainExec() {
    ...
    const useAdjust = CUR_HOST.includes('hoyolab.com')
    ...
    switch (true) {
      ...
      case useAdjust:
        ...
        linkDirect(indexParam, DELAY_TIME.slow);
        break;
      case isJianshu:
        indexParam = INDEX_TO;
    ...
    // eslint-disable-next-line no-undef
      GM_registerMenuCommand(MenuTitle, () => { linkDirect(indexParam, 0); }, 'R');
    }
  }
  mainExec();
})();
