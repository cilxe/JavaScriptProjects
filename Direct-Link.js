...
This script has not been fully tested yet, so if you encounter broken links replaced by this script, 
you can submit feedback [here](https://greasyfork.org/scripts/463408-direct-link/feedback) or 
[Github Issues](https://github.com/cilxe/JavaScriptProjects/issues).
*/
...
(() => {
  ...
  function linkDirect(directURLParams, delayTime) {
    ...
    timeoutID = setTimeout(() => {
      ...
      for (let i = 0; i < links.length; i += 1) {
        if (linkRegex.test(links[i].hostname)) {
  ...
  (() => {
    ...
    const usingTarget = /([\w.]{0,})(juejin|leetcode|gitee|sspai|gcores|zhihu).(com|cn)/.test(pageHost);
    switch (true) {
      case usingTarget:
